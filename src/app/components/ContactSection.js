    ...
    <section id="contact" className={`py-20 w-full font-sans bg-transparent`}>
      <div className={`mb-12 space-y-4 px-2`}>
        <h2 className={`text-4xl md:text-5xl font-black tracking-tighter text-white uppercase`}>
          Let's <span className={`bg-clip-text text-transparent bg-linear-to-r from-primary-400 via-primary-500 to-purple-600`}>Connect</span>
        </h2>
      ...
      <div className={`grid grid-cols-2 lg:grid-cols-4 gap-6`}>
        {socials.map((social, index) => (
          ...
          >
            <div className={`relative h-full p-6 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-[50px]
                            transition-all duration-500 ease-out overflow-hidden shadow-2xl
                            hover:bg-white/8 hover:border-white/20 hover:-translate-y-1.5`}>
              ...
              <div className={`flex items-start justify-between mb-8`}>
                <div className={`p-3.5 rounded-2xl bg-[#0a0a0a] border border-white/10 text-slate-500 
                                transition-all duration-500 shadow-2xl scale-100 group-hover:scale-110 
                                group-hover:border-primary-500/40 ${social.hoverColor}`}>
                  {social.icon}
                </div>
                <ArrowUpRight size={20} className={`text-slate-700 transition-all duration-500 
                                                  group-hover:text-primary-400 group-hover:translate-x-1 group-hover:-translate-y-1`} />
              </div>
              
              <div className={`space-y-1`}>
                <h3 className={`text-white font-black text-xl tracking-tight`}>{social.name}</h3>
                <p className={`text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] transition-colors group-hover:text-primary-300`}>
                  {social.label}
                </p>
              </div>

              {/* Hover Bottom Glow Accent */}
              <div className={`absolute inset-x-6 bottom-0 h-px bg-linear-to-r from-transparent via-primary-500/50 to-transparent 
